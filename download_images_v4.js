...
const downloadImage = async (url, filepath) => {
  try {
    // Try without specific headers first, just standard fetch
    let response = await fetch(url);
    
    if (!response.ok) {
       // Retry with headers if failed
       console.log(`Retrying ${url} with headers...`);
    ...
    // We can't easily create a valid image, but we can delete the file so it doesn't exist as 0 bytes
